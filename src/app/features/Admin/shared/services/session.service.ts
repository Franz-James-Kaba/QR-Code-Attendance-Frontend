...
import { CreateSessionRequest, Session, SessionFilter, SessionListResponse, SessionStatus, UpdateSessionRequest } from '../models/session/session.model';
...
export class SessionService {
  ...
  getSessions(filter?: SessionFilter): Observable<SessionListResponse> {
    ...
    return this.http.get<SessionListResponse>(`${this.adminApiUrl}`, { params })
      .pipe(
  ...
  getSessionById(id: string): Observable<Session> {
    return this.http.get<Session>(`${this.adminApiUrl}/${id}`)
      .pipe(
  ...
  createSession(session: CreateSessionRequest): Observable<Session> {
    return this.http.post<Session>(`${this.adminApiUrl}`, session)
      .pipe(
  ...
  updateSession(sessionId: string, updates: UpdateSessionRequest): Observable<Session> {
    return this.http.put<Session>(`${this.adminApiUrl}/${sessionId}`, updates)
      .pipe(
  ...
  cancelSession(sessionId: string): Observable<Session> {
    return this.http.put<Session>(`${this.adminApiUrl}/${sessionId}/cancel`, {})
      .pipe(
        catchError(error => throwError(() => new Error(`Error cancelling session: ${error.message}`)))
      );
  }
  ...
  generateQrCode(sessionId: string, width: number = 300, height: number = 300): Observable<string> {
    ...
    return this.http.post<Blob>(
      ...
      catchError(error => throwError(() => new Error(`Error generating QR code: ${error.message}`)))
    );
  }
